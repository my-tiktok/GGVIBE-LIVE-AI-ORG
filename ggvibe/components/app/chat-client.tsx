...
export function ChatClient() {
  ...
  const onSend = async (event: FormEvent) => {
    ...
    if (!input.trim() || aiUnavailable) return;
    ...
    try {
      ...
      if (!response.ok) {
        if (payload?.error === 'missing_ai_env') {
          setAiUnavailable(true);
          setMessages((prev) => [
            ...prev,
            {
              role: 'assistant',
              text: 'AI is not configured yet. Messaging is temporarily unavailable while setup is completed.',
            },
          ]);
          return;
        }
    ...
    <div style={{ maxWidth: 860 }}>
      ...
      {aiUnavailable && (
        <div style={{ marginBottom: 12, border: '1px solid rgba(237,186,49,.35)', borderRadius: 10, padding: 10, background: 'rgba(237,186,49,.08)' }}>
          AI not configured yet. You can still browse Marketplace, Wallet, and Seller tools.
        </div>
      )}
      ...
      <form onSubmit={onSend} style={{ display: 'flex', gap: 8 }}>
        <input
          ...
          placeholder={aiUnavailable ? 'AI setup pending' : 'Type your message...'}
          style={{ flex: 1 }}
          disabled={aiUnavailable}
        />
        <button type="submit" disabled={loading || aiUnavailable}>{loading ? 'Sending...' : 'Send'}</button>
      </form>
    </div>
  );
}
