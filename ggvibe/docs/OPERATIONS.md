...
```
/ (repo root)          — NO package.json, NO node_modules, NO lockfile
├── ggvibe/            — Next.js app (production workload)
│   ├── package.json
│   └── package-lock.json
├── ggvibe_mobile/     — Flutter client (NOT part of Replit deploy)
└── scripts/
    └── guard-root.sh  — Build guard: removes stale root artifacts
```

### Why No Root package.json

Replit's package manager (UPM) auto-detects `package.json` and runs `npm install`
at whatever directory it finds one. A root `package.json` triggers a redundant install
that doubles memory usage and causes OOM (exit code 134 / SIGABRT) on constrained
deployment environments. The solution: no root `package.json` exists; all Node
dependencies live exclusively under `ggvibe/`.

### Build Guard

`scripts/guard-root.sh` runs as the first step of the deploy build command.
It removes any stale root artifacts (package.json, lockfiles, node_modules)
that might be regenerated by tooling, then verifies `ggvibe/package.json`
and `ggvibe/package-lock.json` exist before proceeding.

### Deploy Commands

```
build: bash scripts/guard-root.sh && cd ggvibe && npm ci && npm run build
run:   cd ggvibe && npm run start
```

`NODE_OPTIONS='--max-old-space-size=1536'` is set during build to prevent OOM.
