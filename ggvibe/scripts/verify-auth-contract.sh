...
SESSION_SECRET="${SESSION_SECRET:-verify-secret-verify-secret-1234567890}"
...
OPENAI_APPS_CHALLENGE_TOKEN="${OPENAI_APPS_CHALLENGE_TOKEN:-openai-challenge-token}"

NEXTAUTH_SECRET="$NEXTAUTH_SECRET" SESSION_SECRET="$SESSION_SECRET" NEXTAUTH_URL="$NEXTAUTH_URL" NEXT_PUBLIC_APP_URL="$NEXT_PUBLIC_APP_URL" \
...
if [[ -z "$challenge_body" ]]; then
  echo "OpenAI challenge body must not be empty" >&2
  exit 1
fi
